var widget = (function(){
	...
	var slider = function(parameter){
		...
		var click = function(x) {
			...
			if(showValue){
				...
				d3.select("#slider_"+this.id()+".slider").select(".tag").text(this.name()+" = "+nf(this.value()))
			}
			update("start");
		}
		...
		return {
			...
			showValue: function(arg) { if ("undefined" === typeof arg) { return showValue } else { showValue = arg; return this }},
	...
	var sliderElement = function(d,i) {
		...
		s.append("line")
			...
			.call(d3.drag()
				.on("start drag", function() {
					...
					if(d.showValue()){
						s.select(".tag").text(d.name()+" = "+nf(d.value()))
					}
					d.update("start");
				})
				.on("end", function() {
		...
		if(d.name()!=""){					
			...
			s.append("text").text(d.showValue() ? d.name()+" = "+nf(d.value()) : d.name())
				.attr("class", "tag")
				.style("text-anchor",anchor)
				.style("alignment-baseline",valign)
				.style("font-size",d.fontSize())
	...
	function symbol(type,scale){
			...
			default:
				return function(){
					var p = d3.path();
					p.arc(-2*scale,0,scale,0,2*Math.PI,true);
					p.closePath();
					return p.toString();
				};
